...
ganaste = pygame.image.load('Proy-Pucham-n-main/Proyecto Puchamon/img/PANTALLA DE GANASTE.png')
perdiste = pygame.image.load('Proy-Pucham-n-main/Proyecto Puchamon/img/PANTALLA DE PERDISTE.png')
...
def cambiar_musica(ruta_musica, repetir=-1, volumen=0.1):
...
turnos_boost_enemigo = 0 
enemigo_reduce_daño = False  
turnos_reducir_daño = 0 
...
def calcular_daño(ataque, nombre_atacante, nombre_defensor, es_jugador=True):
    ...
    if ataque == "Gruñir":
        if enemigo_reduce_daño:
            turnos_reducir_daño = 2 
        else:
            enemigo_reduce_daño = True 
            turnos_reducir_daño = 2 
    ...
    if ataque == "Motivación":
        ...
        if es_jugador:
            jugador_hp = min(jugador_hp + cura, 100)  
        else:
            enemigo_hp = min(enemigo_hp + cura, 100)  
...
def mostrar_ataques(ataques_jugador, screen_width, y):
    for i, ataque in enumerate(ataques_jugador):
        ...
        x = (screen_width // 2) - 200 + columna * 250  
